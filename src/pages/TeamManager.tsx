...
//  ChiEAC
//
//  Team management page with modular TeamManager component
...
export default function TeamManager() {
  ...
  // States for duplicate team name detection
  const [showDuplicateTeamWarning, setShowDuplicateTeamWarning] = useState(false);
  const [duplicateTeamData, setDuplicateTeamData] = useState<{existingTeam: Team | null, newTeamData: any}>({
    existingTeam: null,
    newTeamData: {}
  });
  ...
  const generateTeamFields = (teamName: string) => {
    const trimmedName = teamName.trim(); // Remove leading/trailing whitespace
    const uniqueId = generateUniqueTeamId(trimmedName);
    // Extract the team code from the unique ID (remove 'team.' prefix)
  ...
  const generateUniqueTeamId = (baseName: string): string => {
    const trimmedName = baseName.trim(); // Remove leading/trailing whitespace
  ...
  const generateUniqueMemberId = (memberName: string, teamCode: string): string => {
    const trimmedName = memberName.trim(); // Remove leading/trailing whitespace
  ...
  // Handle duplicate team name confirmation
  const confirmDuplicateTeamCreation = () => {
    if (duplicateTeamData.newTeamData) {
      // Generate a unique team code for the duplicate name
  ...
  const handleTeamSubmit = async (e: React.FormEvent) => {
    ...
    if (editingTeam) {
      ...
      // For new teams, check for duplicates first
      const existingTeam = findTeamByName(teamFormData.team_name);
      if (existingTeam) {
        // Found duplicate, generate auto fields and show warning
        const autoGeneratedFields = generateTeamFields(teamFormData.team_name);
      ...
      // No duplicate found, proceed with creation confirmation
      const teamData = {
        ...teamFormData,
        team_code: autoGeneratedFields.team_code,
        id: autoGeneratedFields.id,
  ...
  const handleMemberDragEnd = async (result: DropResult) => {
    if (!result.destination || !isReorderingMode) return;
  ...
  const enterReorderingMode = () => {
    setOriginalMemberOrder([...filteredMembers]);
  ...
  const cancelReordering = () => {
    setFilteredMembers([...originalMemberOrder]);
    exitReorderingMode();
  };

  const hasOrderChanged = () => {
    if (originalMemberOrder.length !== filteredMembers.length) return true;
    return originalMemberOrder.some((originalMember, index) => 
  ...
  const handleTeamDragEnd = async (result: DropResult) => {
    if (!result.destination || !isTeamReorderingMode) return;
  ...
  const cancelTeamReordering = () => {
    setTeams([...originalTeamOrder]);
    exitTeamReorderingMode();
  };

  const hasTeamOrderChanged = () => {
    if (originalTeamOrder.length !== teams.length) return true;
    return originalTeamOrder.some((originalTeam, index) => 
  ...
  return (
    <div>
            ...
            <div className="flex items-center gap-4">
              ...
              {teams.length > 1 && (
                <div className="flex items-center gap-3">
                  {!isTeamReorderingMode ? (
                    <button
                      onClick={enterTeamReorderingMode}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={handleDoneTeamReordering}
              ...
              <button
                onClick={() => {
                  ...
                  setSelectedTeamForView(null); // Clear any selected team
                  setTeamRightPanelMode('add'); // Set to add mode instead of showing form inline
                }}
          ...
          <div className={`flex gap-6 min-h-[600px] ${selectedTeamForView || teamRightPanelMode ? '' : 'justify-center'}`}>
            ...
            <div className={`${selectedTeamForView || teamRightPanelMode ? 'w-[70%]' : 'w-full'} transition-all duration-300`}>
              <DragDropContext onDragEnd={handleTeamDragEnd}>
                <Droppable droppableId="teams" isDropDisabled={!isTeamReorderingMode}>
                  {(provided) => (
                    <div
                      {...provided.droppableProps}
                      ref={provided.innerRef}
                      className={isTeamReorderingMode 
                        ? "space-y-3" 
                        : `grid grid-cols-1 ${selectedTeamForView || teamRightPanelMode ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'} gap-6`
                      }
                    >
                      {teams.map((team, index) => {
                        const isSelected = selectedTeamForView?.id === team.id;
                        return (
                          <Draggable key={team.id} draggableId={team.id} index={index} isDragDisabled={!isTeamReorderingMode}>
                            {(provided, snapshot) => (
                              <div
                                ref={provided.innerRef}
                                {...provided.draggableProps}
                                {...(isTeamReorderingMode ? provided.dragHandleProps : {})}
                              >
                                {isTeamReorderingMode ? (
                                  // Rectangle style for reordering (like member cards)
                                  <div
                                    className={`group relative backdrop-blur-sm border rounded-xl p-6 transition-all duration-300 shadow-lg ${
                                      isTeamReorderingMode 
                                        ...
                                        : 'cursor-pointer'
                                    } ${snapshot.isDragging ? 'shadow-2xl shadow-blue-500/30 scale-105' : ''}`}
                                  >
              ...
              <div className="w-[30%]">
                      ...
                      <div className="space-y-3 mt-6">
                        ...
                        <div className="flex space-x-3">
                          <button
                            ...
                            className="flex-1 bg-slate-700/80 backdrop-blur-sm text-slate-200 px-4 py-2 rounded-lg font-medium hover:bg-slate-600/80 hover:text-white transition-all duration-300 border border-slate-600/50 hover:border-slate-500/50"
                          >
                            Edit Details
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeleteTeam(selectedTeamForView.id);
                            }}
                            className="flex-1 bg-red-600/80 backdrop-blur-sm text-white px-4 py-2 rounded-lg font-medium hover:bg-red-700/80 transition-all duration-300"
                          >
                            Delete Team
                          </button>
                        </div>
                ...
                {teamRightPanelMode === 'add' && (
                  <div className="p-6 rounded-xl bg-gradient-to-br from-slate-900/80 to-slate-800/60 backdrop-blur-sm border border-orange-500/50 shadow-2xl shadow-orange-500/20 sticky top-6">
                    ...
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-6">
                        <h3 className="font-semibold text-white text-lg">Add New Team</h3>
                        <button
                          onClick={closeTeamRightPanel}
                          className="text-slate-400 hover:text-white transition-colors duration-200"
                        >
        ...
        <div>
          ...
          <div className="bg-gradient-to-r from-slate-800/80 to-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 shadow-xl mb-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    {isReorderingMode 
                      ? `Drag to reorder • ${filteredMembers.length} ${filteredMembers.length === 1 ? 'member' : 'members'}`
              ...
              <div className="flex items-center gap-4">
                {/* Reordering controls */}
                {selectedTeam && filteredMembers.length > 1 && (
                  <div className="flex items-center gap-3">
                    {!isReorderingMode ? (
                      <button
                ...
                {selectedTeam && (
                  <button
                    onClick={() => {
                      ...
                      setEditingMember(null);
                      setShowMemberForm(true);
                      setRightPanelMode('edit'); // This was missing!
                    }}
                        ...
                        <Draggable key={member.id} draggableId={member.id} index={index} isDragDisabled={!isReorderingMode}>
                          {(provided, snapshot) => (
                            <div
                              ref={provided.innerRef}
                              {...provided.draggableProps}
                              {...(isReorderingMode ? provided.dragHandleProps : {})}
                              onClick={() => !isReorderingMode && handleMemberCardClick(member)}
                              className={`group relative backdrop-blur-sm border rounded-xl p-6 transition-all duration-300 shadow-lg ${
                                isReorderingMode 
                                  ? 'cursor-grab active:cursor-grabbing border-blue-500/50 bg-slate-900/70' 
                                  : 'cursor-pointer'
      ...
      {/* Order Change Confirmation Popup */}
        ...
        <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-lg border border-blue-500/30 max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-slate-200">Confirm Order Changes</h3>
            </div>
            
            <div className="space-y-4 mb-6">
              <p className="text-slate-300 text-sm">
                You have changed the order of team members. Do you want to publish these changes?
              </p>
              ...
              <div className="bg-blue-900/30 p-3 rounded-lg border border-blue-700/30">
                <p className="text-xs text-blue-300 text-center font-medium">
                  The new order will be saved and visible in the app immediately.
                </p>
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                onClick={() => setShowOrderChangeConfirmation(false)}
                className="flex-1 bg-slate-600 text-slate-200 px-4 py-3 rounded-lg font-medium hover:bg-slate-700 transition-all duration-300"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 py-3 rounded-lg font-bold hover:from-blue-600 hover:to-indigo-600 transition-all duration-300"
              >
                Publish Changes
              </button>
            </div>
      ...
      {/* Team Order Change Confirmation Popup */}
        ...
        <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-lg border border-orange-500/30 max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-slate-200">Confirm Team Order Changes</h3>
            </div>
            
            <div className="space-y-4 mb-6">
              <p className="text-slate-300 text-sm">
                You have changed the order of teams. Do you want to publish these changes?
              </p>
              ...
              <div className="bg-orange-900/30 p-3 rounded-lg border border-orange-700/30">
                <p className="text-xs text-orange-300 text-center font-medium">
                  The new team order will be saved and visible in the app immediately.
                </p>
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                onClick={() => setShowTeamOrderChangeConfirmation(false)}
                className="flex-1 bg-slate-600 text-slate-200 px-4 py-3 rounded-lg font-medium hover:bg-slate-700 transition-all duration-300"
              >
                Cancel
              </button>
              <button
                onClick={confirmTeamOrderChanges}
                className="flex-1 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-4 py-3 rounded-lg font-medium hover:from-orange-600 hover:to-amber-600 transition-all duration-300 shadow-lg"
              >
                Publish Changes
              </button>
            </div>
      ...
      {showDuplicateTeamWarning && duplicateTeamData.existingTeam && (
        <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-lg border border-amber-500/30 max-w-lg w-full mx-4 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center">
                <svg className="w-6 h-6 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.864-.833-2.634 0L3.179 16.5c-.77.833.192 2.5 1.732 2.5z" />
                </svg>
              </div>
              <h3 className="text-lg font-semibold text-amber-400">Duplicate Team Name</h3>
            </div>
            
            <div className="space-y-4 mb-6">
              <div className="bg-amber-900/20 p-4 rounded-lg border border-amber-700/30">
                <p className="text-slate-300 text-sm font-medium mb-3">
                  A team with the name "{duplicateTeamData.newTeamData?.team_name}" already exists.
                </p>
                
                <div className="bg-slate-700/50 p-3 rounded-lg mb-4">
                  <h4 className="text-amber-300 font-medium text-sm mb-2">Existing Team:</h4>
                  <div className="space-y-1 text-sm text-slate-300">
                    <div><span className="text-slate-400">Name:</span> {duplicateTeamData.existingTeam.team_name}</div>
                    <div><span className="text-slate-400">Description:</span> {duplicateTeamData.existingTeam.team_description}</div>
                    <div><span className="text-slate-400">Team Code:</span> {duplicateTeamData.existingTeam.team_code}</div>
                  </div>
                </div>

                <div className="bg-green-900/20 p-3 rounded-lg border border-green-700/30 mb-4">
                  <h4 className="text-green-300 font-medium text-sm mb-2">New Team Will Be Created With:</h4>
                  <div className="space-y-1 text-sm text-slate-300">
                    <div><span className="text-slate-400">Name:</span> {duplicateTeamData.newTeamData?.team_name}</div>
                    <div><span className="text-slate-400">Description:</span> {duplicateTeamData.newTeamData?.team_description}</div>
                    <div><span className="text-slate-400">Team Code:</span> <span className="text-green-300 font-medium">{duplicateTeamData.newTeamData?.team_code}</span></div>
                  </div>
                </div>

                <p className="text-slate-300 text-sm">
                  The new team will have a unique team code to avoid conflicts with the existing team.
                </p>
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                onClick={cancelDuplicateTeamCreation}
                className="flex-1 bg-slate-600 text-slate-200 px-4 py-3 rounded-lg font-medium hover:bg-slate-700 transition-all duration-300"
              >
                Cancel
              </button>
              <button
                onClick={confirmDuplicateTeamCreation}
                className="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-3 rounded-lg font-medium hover:from-amber-600 hover:to-orange-600 transition-all duration-300 shadow-lg"
              >
                Create Anyway
              </button>
            </div>
