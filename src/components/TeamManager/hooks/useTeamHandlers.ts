...
export const createTeamHandlers = (
  ...
  const disableTeamExpertMode = () => {
    if (editingTeamFirestoreFields) {
      ...
      // Don't override user's expert mode edits - preserve the current autoGeneratedFields values
    }
  };
  ...
  const disableMemberExpertMode = () => {
    if (editingMemberFirestoreFields) {
      ...
      // Don't override user's expert mode edits - preserve the current memberAutoGeneratedFields values
    }
  };
  ...
  const executeTeamUpdate = async () => {
    try {
      if (editingTeam) {
        ...
        await updateDoc(doc(db, 'teams', editingTeam.id), updatedTeamData);
        
        // Update selectedTeam if it's the same team being edited
        if (selectedTeam && selectedTeam.id === editingTeam.id) {
          const updatedTeam = { ...selectedTeam, ...updatedTeamData };
